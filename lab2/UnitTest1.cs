namespace testUnit
{
    ...
    public class GalleryTests
    {
        ...
        [TestMethod]
        public void RequestPainting_ValidParameters_ReturnsRequestedPainting()
        {
            // Arrange
            var gallery = new Gallery();
            ...
            // Act
            var painting = gallery.RequestPainting(artist, title, technique, size);

            // Assert
            Assert.AreEqual(title, painting.Title);
            Assert.IsTrue(painting.Authors.Contains(artist));
            Assert.AreEqual(technique, painting.Technique);
            Assert.AreEqual(size, painting.Size);
        }
